...
describe("appState", function() {
  it("should handle busy / ready", function() {
    expect(appState().store.getState().state.ready).toBe(0);
    ...
    expect(appState().store.getState().state.ready).toBe(1);
    expect(appState().store.getState().state.max).toBe(1);

    appState().actions.state.busy("test");
    expect(appState().store.getState().state.ready).toBe(2);
    expect(appState().store.getState().state.max).toBe(2);

    appState().actions.state.busy("test");
    expect(appState().store.getState().state.ready).toBe(3);

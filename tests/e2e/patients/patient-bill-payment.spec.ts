...
test("bill.payment", async ({ page }) => {
  ...
  // API Add payment, check total
  const editId = await cryptomedic.apiCrudCreate(`/fiche/payment`, {
    bill_id: billId,
    amount: 150
  });
  ...
  }
  {
    // GUI payment edit
    const newData = {
      Date: e2eDefaultDate(2),
      Amount: "175",
      Comments: "e2e updated"
    };
    const paymentEditPanel = new E2EIOPanel(
      paymentPanel.getByTestId(`payment.${editId}`),
      paymentFieldsConfig
    );
    await paymentEditPanel.doOpen();
    await paymentEditPanel.doEdit();
    await paymentEditPanel.expectAllInputValues({
      Amount: "150"
    });
    await paymentEditPanel.setAllInputValues(newData);
    await paymentEditPanel.doSave();
    await paymentEditPanel.doOpen();
    await paymentEditPanel.expectAllOutputValues(newData);

    await billPanel.expectOutputValue(totalPaymentsLabel, "775.0");

    // GUI payment Delete
    await paymentEditPanel.doEdit();
    await paymentEditPanel.doDelete();

    await billPanel.expectOutputValue(totalPaymentsLabel, "600.0");
  }
});

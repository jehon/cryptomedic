import XInputPicture from "../../../legacy/app-old/v1/elements/x-input-picture.js";
...
describe("tests/unit/x-input-picture-test.js", function () {
  ...
  const invalidImg = "data:text/html,bonjour";

  webDescribe(
    "initialized",
    "<x-input-picture></x-input-picture>",
    function (element) {
      ...
      it("should load a file and set", function (done) {
        element()._generatePreview(
          XInputPicture.dataURItoBlob(img, "test.jpg")
        );
        ...
        setTimeout(() => {
          ...
          expect(element().value.length).toBeGreaterThan(10);
          done();
        }, 100);
      });

      it("should load a file and shrink", function (done) {
        element().maxSize = 50;
        element()._generatePreview(
          XInputPicture.dataURItoBlob(img, "test.jpg")
        );
        ...
        setTimeout(() => {
          ...
          expect(element().value).toBeTruthy();
          expect(element().value.length).toBeGreaterThan(10);
          expect(element().value.length).toBeLessThan(10 * 1000);
          done();
        }, 100);
      });
    }

	...
	webDescribe('initialized', '<x-file></x-file>', function (element) {
		...
		describe('with data', function () {
			...
			it('with assertNumeric', function () {
				...
				expect(() => element().assertNumeric('b')).toThrowError('Data b is not numeric(azer)');
			...
			it('with assertNumericNotZero', function () {
				...
				expect(() => element().assertNumericNotZero('b')).toThrowError('Data b is not numeric(azer)');
				expect(() => element().assertNumericNotZero('c')).toThrowError('Data c is not non-zero(0)');
			...
			it('with assertDate', function () {
				...
				expect(() => element().assertDate('b')).toThrowError('Data b is not a valid date(azer)');
				expect(() => element().assertDate('c')).toThrowError('Data c is not a valid date(0)');
				expect(() => element().assertDate('d')).toThrowError('Data d is not a valid date(1)');
				expect(() => element().assertDate('obj')).toThrowError('Data obj is not a valid date(object)');

				expect(() => element().assertDate('dateNumInv')).toThrowError('Data dateNumInv is not a valid date(20)');

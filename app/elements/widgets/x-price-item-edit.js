...
export default class XPriceItemEdit extends HTMLElement {
    ...
    constructor() {
        ...
        this.shadowRoot.append(
            createElementWithTag('div', {}, [
                this.elChoiceOpen = /** @type {HTMLInputElement} */ (createElementWithTag('input', { type: 'radio', name: 'choice', choice: 'open' }, '',
                    el => el.addEventListener('changed', () => this.choiceChanged()))),
                createElementWithTag('span', {}, 'Open'),

                this.elChoiceUnused = /** @type {HTMLInputElement} */ (createElementWithTag('input', { type: 'radio', name: 'choice', choice: 'unused' }, '',
                    el => el.addEventListener('changed', () => this.choiceChanged()))),
                createElementWithTag('span', {}, 'Not used'),

                this.elChoiceSpecific = /** @type {HTMLInputElement} */ (createElementWithTag('input', { type: 'radio', name: 'choice', choice: 'specified' }, '',
                    el => el.addEventListener('changed', () => this.choiceChanged()))),
                createElementWithTag('span', {}, 'Specific'),
            ]),
            this.elSpecific = /** @type {HTMLInputElement} */ (createElementWithTag('input', { type: 'number', min: 2 }))
        );
    ...
    choiceChanged() {
        if (this.elSpecific) {
            this.elSpecific.style.display = (this.elChoiceSpecific.checked) ? '' : 'none';
        }
    }

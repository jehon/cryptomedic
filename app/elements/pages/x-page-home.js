
import '../components/x-patient-by-reference.js';

import { defineCustomElement } from '../../js/custom-element.js';
import { getRouteToCreateReference } from '../../js/router.js';

/**
 *
 */
export default class XPageHome extends HTMLElement {
    constructor() {
        super();
        this.innerHTML = `
<div class='container-fluid' id='page_home'>
    <h1 class='text-center'><img src='/static/img/home.gif'>Home</h1>
    <div class='row'>
        <div class='col-sm-6'>
            <x-patient-by-reference></x-patient-by-reference>
        </div>
        <div class='col-sm-6'>
            <x-group-panel title='Create a reference for me'>
                <div white>
                    If you want to create a new patient with an <b>autogenerated</b>
                    entry order, choose this option.<br>
                    On the next page, the system will ask you what year you want to use.<br>
                    <x-button action='commit' to-route='${getRouteToCreateReference()}' id='button_generate_reference'>
                        Generate a reference for me
                    </x-button>
                </div>
            </x-group-panel>
        </div>
    </div>
</div>
`;

        this.classList.add('full');
    }
}

defineCustomElement(XPageHome);

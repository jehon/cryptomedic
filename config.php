...
/* Include secrets */
require_once(__DIR__ . DIRECTORY_SEPARATOR . "globalConfig.php");
...
$config['authenticate.loginRequest'] = 'SELECT users.username as login, users.group as `group`, users.id as id FROM users '
    . ' WHERE username = :username and password = SHA1(concat("' .  getGlobalConfig('authenticateSalt') . '", :password))';
$config['authenticate.updatePasswordRequest'] = 'UPDATE users SET password = SHA1(concat("' . getGlobalConfig('authenticateSalt') .'", :password)) WHERE id = :id';
